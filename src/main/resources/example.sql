...
(
    id      BIGSERIAL PRIMARY KEY,                         -- NOT NULL не нужно, PK итак это предполагает
    name    TEXT NOT NULL,
    price   INT  NOT NULL CHECK ( price >= 0 ),            -- kopecks
    qty     INT  NOT NULL CHECK ( qty >= 0 ) DEFAULT 0,
    removed BOOL NOT NULL                    DEFAULT FALSE, -- признак, удален или нет
...
(
    id      BIGSERIAL PRIMARY KEY,
    manager_id BIGINT REFERENCES managers,
    created timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
-- в данном случае AS обязательно
-- используется только на SELECT
CREATE VIEW product_sale_stats AS
